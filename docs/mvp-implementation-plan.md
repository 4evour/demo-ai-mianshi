...
2026-07-13 已完成 OCR 服务代码、OCR 分流与人工纠错、动态追问状态机、逐题 API、候选人页面、审核时间线、报告证据校验和审核员多页面审阅。Python 单元测试、48 项 Node 测试和 Next.js 生产构建通过。Docker CLI 已安装，但本机 Docker Desktop 未启动 Linux 引擎，`docker build` 的实际镜像验证仍待宿主引擎恢复后执行。

## 概述

在现有 `screening-mvp` 主链路上补齐已确认的 P0 能力：文本 PDF 失败时使用本地 PaddleOCR 降级识别；候选人采用聚焦式逐题答题，并由受控状态机执行 DeepSeek 动态追问；审核员可按多个岗位和候选人分别审阅，并对比同岗位最多两名已生成报告的候选人。实现保持本地 JSON 存储和人工最终决策，不接入 GitHub、认证、数据库或队列。
...
- `components/job-detail-client.tsx`
- `lib/store.ts`
...
- `components/candidate-workflow.tsx`
