...
- 审核员多页面审阅：岗位列表、岗位详情、候选人档案、问答记录、报告和同岗位双人对比。
...
- 审核员页面使用同源 API，并按岗位、候选人和报告拆分页面；候选人页面只通过随机 token 访问对应会话。
...
审核员从候选人档案进入独立问答页面查看完整时间线，区分主问题、AI 追问和候选人回答。长回答默认折叠，每条追问显示触发原因；模型降级跳过的节点显示“需要人工复核”，不能伪装为正常判断。

## 审核员页面

- `/`：岗位列表及候选人、已提交会话和已生成报告数量。
- `/jobs/new`：新建岗位。
- `/jobs/:jobId`：岗位详情、简历上传、OCR 修正、候选人状态筛选和同岗位双人选择。
- `/candidates/:candidateId`：候选人档案、项目经历、问题生成、编辑和发布。
- `/candidates/:candidateId/interview`：独立问答记录，长回答默认折叠。
- `/candidates/:candidateId/report`：独立报告生成、证据审阅和人工决策。
- `/jobs/:jobId/compare?candidateIds=:id1,:id2`：最多两名已生成报告的同岗位候选人横向对比。

岗位、候选人、问答和报告入口均在新标签页打开，保留原列表页，便于审核员并行审阅。
...
- `GET /api/jobs`：读取岗位列表。
- `GET /api/jobs/:jobId`：读取岗位详情和候选人流程摘要。
- `GET /api/candidates/:candidateId`：读取脱敏后的候选人档案、会话和报告。
...
- `POST /api/reports`：调用 DeepSeek 生成评分报告。
